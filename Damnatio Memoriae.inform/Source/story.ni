...
It is a fast, timed game, taking only a few minutes to play once, but probably requiring multiple attempts to bring to a satisfactory conclusion." The release number is 5.
...
A thing has a property called fiery description. The fiery description of a thing is usually "is consumed in phantom flames, and the link dies with it". The fiery description of an impervious thing is usually "catches impossible fire and turns into a cloud of thick, formed smoke before dissipating. The link too is gone, of course".
...
Instead of burning something flammable which affects something important (called the slave):
	if the slave is impervious and the slave protects the noun, say "[The noun] resists flame, thanks to its link with [the slave]." instead;
	if the slave affects the noun
	begin;
...
Understand "attack [something] with [something held]" as attacking it with.
...
A thing has a property called shattering description. The shattering description of a thing is usually "shatters under an invisible blow, and the link dies with it". 
...
After reverse linking an inscribed thing to an inscribed thing:
	...
	begin;
		now the second noun does not incriminate all the people incriminated by the second noun;
	...
	begin;
		now the noun does not incriminate all the people incriminated by the noun;
	...
	otherwise say "[The noun] innocently mentions [the list of people mentioned by the second noun], ";
	if the second noun incriminates someone, say "and [the second noun] incriminates [the list of people incriminated by the second noun].";
	otherwise say "and [the second noun] mentions [the list of people mentioned by the noun].". 

Book 3 - Identification Issues

A person can be introduced or strange. A person has a property called a tag. The tag of a person is usually "no one of note".
...
Instead of examining the ruined pitcher, say "Poor thing, it's nothing but painted shards now."
...
Before taking Clemens when the number of things carried by the player > 0:
	if  Clemens is carried by the player, continue the action;
...
Before taking something when the player is carrying Clemens:
	if the noun is Clemens, continue the action;
...
And to make matters worse, someone has written your name so it seems to be emerging out of the mouth of Achilles, on the other side, as though he's calling out to you." instead;
	...
	otherwise; 
		if the vase mentions someone (called the grafitee), let the character be the grafitee;
		otherwise let the grafitee be Clemens;
	end if;
	say "A Greek krater with Achilles on one side and the Fates on the other. 

One night in a drunken fling Clemens scratched some writing into the black field behind [if the grafitee is a woman]Clotho[otherwise]Achilles[end if]. So now it says [italic type][the grafitee] est kal[if the grafitee is a woman]e[otherwise]os[end if][roman type] in a pig mixture of Greek letters and Roman. Roughly, [italic type][the grafitee] is hott[end if][roman type][if the grafitee is Julia]. Kind of a kinky thing to have lying about the house referring to your own mother[end if]."
...
Instead of examining the recent letter when something which protects the recent letter mentions Clemens:
	if something which protects the recent letter mentions Julia, continue the action;
...
After printing the name of a paper (called the document) while taking inventory:
	...
	begin;
		if the number of people incriminated by the document is 1, say " from [a random person incriminated by the document]";
		otherwise say " from [a random other person incriminated by the document]";
	otherwise;
		...
		begin;
			...
			otherwise say " from [a random other person mentioned by the intertext]";
		otherwise;
...
A liquid is a kind of thing. Before labor when the noun is a liquid or the second noun is a liquid: say "You have no such powers over liquids." instead. A liquid has a property called the spilling description. The spilling description of a liquid is usually "spills out on the ground, and soon soaks in".
...
An endgame rule: 
	...
	begin; 
		...
		if a visible thing incriminates the player, say "Involuntarily you find yourself glancing toward [the list of visible things which incriminate the player].[line break]";
...
An endgame rule:
	...
	begin;
		if started printing is 0, say "[start]'Found him,' announces one of the military voices, grimly satisfied.
...
But you aren't dead. Augustus and his warnings bought you that much, at least.";
		otherwise say "Clemens gives an indignant shout as though someone has prodded him with a sharp thing. 'You,' says one of the guard. 'Wake up: Tiberius sent us to work on you.'
...
An endgame rule:
	if the player is apparently guilty, make no decision;
	if Clemens is silent, make no decision;
...
Rule for deciding the concealed possessions of the player: if the particular possession incriminates the player, yes.
...
Later, they go out, even expressing a little distress at having had to kill you. Then you get up, and remove the spear, and wander around to where the Clemens corpse is, bleeding from the torso."; end the game saying "You have faked your own death";
	rule succeeds;
					end if;
				end if;
			end if;
...
An endgame rule:
	if the soldiers can see the player and the player is apparently innocent
	begin; 
		if started printing is 0, say "[start]They search impatiently, sometimes glancing at you as though puzzled by your demeanor. You smile at them blandly.[line break]";
...
There is a further conversation about what this resemblance means, but eventually they draw some ribald conclusions about your mother, and withdraw.";
		otherwise say "The soldiers confer among themselves in low voices for another minute, then turn to you. 'Tiberius sends his wishes for your continued health after the death of your grandfather,' says the chief of them, managing somehow not even to smile. 
...
Procedural rule when taking instant action: ignore the turn sequence rules.

Before reading a command while the turn count is 1:
	if taking instant action, do nothing;
...
Annette Edelman, Admiral Jota, Carl Muckenhoupt, and J. Robinson Wheeler beta-tested. Admiral Jota and Jake Eakle reported flaws in release 3 that were fixed for release 4; Dan Shiovitz, Jeremy Douglass, and JosÃ© Manuel GarcÃ­a-Patos, those fixed in release 5."	--
